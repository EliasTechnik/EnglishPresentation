...
procedure tmyjsonnode.set_type(_type: string);
begin
  nodetype:=_type;
end;

procedure tmyjsonnode.parse_array_str(str: ansistring);
var item:tmyjsonnode;
begin
  if nodetype:='array' then begin
    id str[1]<>'"' then delete(str,1,pos(str,'"'));
    while length(str)>0 do begin
      item:=tmyjsonnode.create();
      item.set_type('arrayitem');
      item.set_value(copy(str,1,pos(str,'"')-1));
